...
| `def func_norm_raspr_obr(r,mu,sigma)`         | Эта функция использует параметры нормального распределения (mu, sigma) для получения нормального распределения.|
...
Тогда задача формулируется как адача минимизации среднеквадратичного углового отклонения по вертикали <br/>
